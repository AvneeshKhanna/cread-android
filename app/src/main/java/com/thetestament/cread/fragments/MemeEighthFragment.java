...
public class MemeEighthFragment extends Fragment {
    ...
    @BindView(R.id.img_meme_left)
    AppCompatImageView imgMemeLeft;
    @BindView(R.id.img_meme_right)
    AppCompatImageView imgMemeRight;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        //inflate this view
        return inflater
                .inflate(R.layout.fragment_meme_eighth
                        , container
                        , false);
    }
            ...
            case REQUEST_CODE_OPEN_GALLERY_FOR_MEME:
                if (resultCode == RESULT_OK) {
                    ...
                    ImageHelper.startImageCroppingWith918(getActivity()
                            , MemeEighthFragment.this
                            , data.getData()
                            , getImageUri(mImageType));
                } else {
     ...
     */
    @OnClick(R.id.img_meme_left)
    void imgLeftClick() {
        imgView = imgMemeLeft;
        mImageType = Constant.IMAGE_TYPE_USER_MEME_ONE;
        toggleBottomSheet();
    }
     ...
     * Image click functionality.
     */
    @OnClick(R.id.img_meme_right)
    void imgRightClick() {
        imgView = imgMemeRight;
        mImageType = Constant.IMAGE_TYPE_USER_MEME_TWO;
        toggleBottomSheet();
    }
    ...
    /**
     * Click functionality for lefy textView.
     */
    @OnClick(R.id.tv_left)
    void onTvLeftClick() {
        MemeUtil.showMemeInputDialog(getActivity(), tvLeft);
    }

    /**
     * Click functionality for right textView.
     */
    ...
    private void initViews() {
        ...
        MemeUtil.setImageIfExist(Environment.getExternalStorageDirectory().getPath() + "/Cread/Meme/meme_pic_one.jpg"
                ...
                , imgMemeLeft);
        //Load image if exist
        MemeUtil.setImageIfExist(Environment.getExternalStorageDirectory().getPath() + "/Cread/Meme/meme_pic_two.jpg"
                , getActivity()
                , imgMemeRight);
    }
    ...
    private void getWritePermissionForGallery() {
        ...
        if (Nammu.checkPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            ...
            ImageHelper.chooseImageFromGallery(MemeEighthFragment.this
                    , Constant.REQUEST_CODE_OPEN_GALLERY_FOR_MEME);
        } else {
            //We do not own this permission
            if (Nammu.shouldShowRequestPermissionRationale(MemeEighthFragment.this
                ...
                Nammu.askForPermission(MemeEighthFragment.this
                        , Manifest.permission.WRITE_EXTERNAL_STORAGE
    ...
    private void saveBmpFileToDevice(Bitmap bitmap) {
        try {
            FileOutputStream out = new FileOutputStream(new File(ImageHelper.getImageUri(mImageType).getPath()));
            bitmap.compress(Bitmap.CompressFormat.JPEG, 85, out);
            out.close();
            //Code to launch image cropper
            ImageHelper.startImageCroppingWith918(getActivity()
                    , MemeEighthFragment.this
                    , ImageHelper.getImageUri(mImageType)
                    , ImageHelper.getImageUri(mImageType));
        } catch (IOException e) {
            e.printStackTrace();
            ViewHelper.getSnackBar(rootView, getString(R.string.error_msg_internal));
    ...
    private void validateMemeCreation() {
        ...
        if (tvLeft.getText().toString().equals(getString(R.string.hint_text_meme))
                || tvRight.getText().toString().equals(getString(R.string.hint_text_meme))) {
            ViewHelper.getToast(getContext(), "Text field can't be empty");
        }
        //If empty
        else if (TextUtils.isEmpty(tvLeft.getText().toString().trim())
                || TextUtils.isEmpty(tvRight.getText().toString().trim())) {
            ViewHelper.getToast(getContext(), "Text field can't be empty");
        } else {
